#  -*-coding:utf8 -*-
# 主要使用到了csv、sys、urllib.request和BeautifulSoup4模块，其中csv模块是为了对csv文件的处理，
# urllib.request可以构造http请求，BeautifulSoup4可以解析页面信息
import csv
import sys
...
for day in li: # 便利找到的每一个li
    ...
    # 将最终的获取的天气写入csv文件
with open('C:/Users/Eazon/Desktop/weather.csv', 'a', errors='ignore', newline='') as f:
    f_csv = csv.writer(f)
    f_csv.writerows(["武汉"])
    f_csv.writerows(final)

    # 绘图
bar = pygal.Line(style=style) # 创建折线图
bar.add('最低气温', lows)
bar.add('最高气温', highs)
...
bar.title = "武汉"+'未来七天气温走向图'
bar.x_title = '日期'
bar.y_title = '气温(摄氏度)'
bar.legend_at_bottom = True
bar.show_x_guides = False
bar.show_y_guides = True
bar.render_to_file('C:/Users/Eazon/Desktop/temperate.svg')


bar.render_to_png("C:/Users/Eazon/Desktop/save.png")
print("yes")
